...
#         cur_user_id = ratings[ratings['user_id']
        ...
        for place in places_id:
            places_names.append(
                attractions[attractions['activityId'] == place]['name'].tolist()[0])
            places_categories.append(
                attractions[attractions['activityId'] == place]['category'].tolist()[0])
            places_prices.append(
                attractions[attractions['activityId'] == place]['price'].tolist()[0])

        # Find all books the mock user has 'not' read before using the to_read data
        unvisited = attractions[~attractions['activityId'].isin(places_id)]['activityId']
        unvisited_id = unvisited.tolist()

        # extract the ratings of all the unread books from ratings dataframe
        unseen_with_score = ratings[ratings['activityId'].isin(unvisited_id)]

        # grouping the unread data on book id and taking the mean of the recommendation scores for each book_id
        grouped_unseen = unseen_with_score.groupby('activityId', as_index=False)['Recommendation Score'].max()
        ...
        for place in grouped_unseen['activityId'].tolist():
            ...
            unseen_places_scores.append(
                grouped_unseen[grouped_unseen['activityId'] == place]['Recommendation Score'].tolist()[0])

        # creating a data frame for unread books with their names, authors and recommendation scores
        unseen_places = pd.DataFrame({
            'att_id': grouped_unseen['activityId'].tolist(),
